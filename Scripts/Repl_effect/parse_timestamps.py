...
folder="/home/petar/AutumnSemester/AdvancedSystems/ReplEffect/Timestamps/MaxTps"
...
def parse_repl_factor(backends):
    ...
    for repl_factor in map(str, replication):
        instance = parse_instance(str(backends), repl_factor)
...
def write_logs(instances):
    with open("parsed_timestamps.log", "w") as file:
        file.write("Mean | Median | Percentile (90)\n")
...
def vis_bars_rt(t):
    ...
    with open("parsed_timestamps.log", "r") as file:
        ...
        for b in range(0, 3):
            for l in range(0, 3): 
                ...
                percsh_get[b].append((perch - val))
                percsl_get[b].append((val - percl))
        ...
        for b in range(0, 3):
            for l in range(0, 3): 
                ...
                percsh_set[b].append((perch - val))
                percsl_set[b].append((val - percl))
    ...
    for j, b in zip(range(0, 3), [3, 5, 7]):
        p1 = plt.bar(ind + j*width, vals_get[j], width, color=colors_get[j], yerr=np.array([percsl_get[j], percsh_get[j]]), error_kw=dict(capthick=1.5, lw=1.5))
        p2 = plt.bar(ind + j*width, vals_set[j], width, bottom=vals_get[j], color=colors_set[j], yerr=np.array([percsl_set[j], percsh_set[j]]), error_kw=dict(capthick=1.5, lw=1.5))
        plots.append(p1)
        plots.append(p2)

    plt.legend((plots[0], plots[2], plots[4], plots[1], plots[3], plots[5]), ('GET 1', 'GET N/2', 'GET N', 'SET 1', 'SET N/2', 'SET N'), loc=2, ncol=2, prop={'size':8}, shadow=True, title="Legend", fancybox=True, fontsize=12)
...
def vis_scaleup():
    ...
    with open("scaleup.log", "r") as file:
        for line in file: 
            val = float(line.split(",")[median])
            perc_high = float(line.split(",")[perc_mhigh])
            perc_low = float(line.split(",")[perc_mlow])
            values.append(val)
            err_high.append(perc_high)
            err_low.append(perc_low)

    repl_axis = [1, 2, 3, 4, 5, 7]

    fig, ax = plt.subplots()
    axes = plt.gca()
    axes.set_xlim([0.5,7.5])
    axes.set_ylim([0.0,10.2])
    ax.set_xlabel("Replication factor")
    ax.set_ylabel("Response time (miliseconds)")

    ax.plot(repl_axis, values, '-o', color=colors[0], label="Median")
    ax.plot(repl_axis, err_high, '-^', linestyle='--', color=colors[0], label="75th Percentile")
    ax.plot(repl_axis, err_low, '-^', linestyle='--', color=colors[0], label="25th Percentile")
    ax.plot(repl_axis, [0.99205] * 6, '-o', color=colors[2], label="Ideal implementation")

    plt.legend(loc=2, ncol=1, prop={'size':10}, shadow=True, title="Legend", fancybox=True, fontsize=10)
    ax.get_legend().get_title().set_color("black")
