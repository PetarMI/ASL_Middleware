...
WORK_CONFIG="/home/ivanovpe/memaslap-workloads/smallvalueW"
...
for ((b=3; b < 8; b+=2)); do
    ...
    printf "${L_GREEN}##### Running instance with $b BACKENDS #####${NC}\n" 

    for ((w=0; w < 3; w++)); do
        printf "${GREEN}##### Running instance with WRITES = ${WRITE_RATIO[$w]} #####${NC}\n" 
        
        # Replication of 3 per experiment instance
        for ((r=0; r < 3; r++));do
            printf "${L_BLUE}### Running replication ${r}${NC}\n"
            ...
            for ((p=0; p < 3; p++));do
                ...
                LOG_FILE="wr_${b}_${WRITE_RATIO[$w]}_${p}_${r}_${REPL_LOG}.log"
                # printf "$LOG_FILE \n"
                CONFIG_FILE="${WORK_CONFIG}${WRITE_RATIO[$w]}.cfg"
                #printf "${CONFIG_FILE}\n"
                command="sudo pkill memaslap; cd $MEM_FOLDER; ./memaslap -s 10.0.0.9:9090 -T $VIRTUAL_CLIENTS -c $VIRTUAL_CLIENTS -o 0.99 -S \"1s\" -t \"120s\" -F \"$CONFIG_FILE\" > \"${MEMASLAP_LOGS}/${LOG_FILE}\""
            ...
            TIMESTAMP_LOG="RequestLog_${b}_${WRITE_RATIO[$w]}_${r}_${REPL_LOG}.log"
        ...
        printf "${GREEN}##### Finished writes ${WRITE_RATIO[$w]} #####${NC}\n\n"
    done

    printf "${L_GREEN}##### Finished for $b BACKENDS #####${NC}\n" 
done